...
def sample_one_point_from_error_center(gt_masks, pred_masks, padding=True):
    ...
    prefer_slow_edt = os.name == "nt" or os.getenv("SAM3_FORCE_SLOW_EDT", "0") == "1"
    if prefer_slow_edt:
    ...
    try:
        ...
        if "no available kernel" not in message and "no kernel" not in message:
            raise
        logger.warning(
            "SAM3 EDT Triton kernel unavailable; falling back to slow EDT path: %s",
            exc,
        )
